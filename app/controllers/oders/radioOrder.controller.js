...
exports.UploadResult = async (req,res)=>{
    RadioOrder.findOne({
        ...
        if(fs.existsSync(path.join(__dirname, "../../../public/radios/"+data.result))){
            fs.unlinkSync(path.join(__dirname, "../../../public/radios/"+data.result))
        }
        RadioOrder.update({
...
exports.findByCode = (req, res) => {
    ...
    exports.myfindByPtId = (req,res)=>{
        db2.query("select RO.*,R.name as radioName,RFD.address,RFD.phone,RFD.organization from radioOrders RO JOIN radios R on RO.radioId = R.id JOIN radioFrontDisks RFD on RO.radioFDId=RFD.id where RO.isDeleted = 0 and RO.ptId="+req.body.ptId,function(err,result){
            if(err){
...
exports.findByPtId = (req, res) => {
    db2.query('select D.firstName , D.lastName ,RO.*,R.name as radioName from radioOrders RO JOIN radios R on RO.radioId = R.id JOIN doctors D on RO.drId = D.id where RO.isDeleted=0 and RO.ptId='+req.body.ptId,function(err,result){
            if(err){
                res.status(400).send(err)
            }else{
                res.send(result)
...
exports.findByDrId = (req,res)=>{
    ...
    db2.query("SELECT rs.name as radioName ,Lo.createdAt, Lo.id,Lo.result,Lo.comments,P.id as PtID,P.firstname,P.lastname,P.address,P.phone from radioOrders Lo Join Patients P on Lo.ptId = P.id JOIN radios rs on Lo.radioId = rs.id where Lo.isDeleted = false and  Lo.drId = "+ress.id,function(err,result){
        if(err){
            res.status(500).send(err.message)
        }else{
            res.send(result)
...
exports.findByradioId = (req,res)=>{
    radioFD.findOne({
        ...
        db2.query("SELECT d.firstName as OrderingDrFirstName , d.lastName as tOrderingDrLastName ,  Lo.id,Lo.result,Lo.comments,P.id as PtID,P.phone,P.firstname,P.secondName,P.lastname,P.address,P.phone from radioOrders Lo Join Patients P on Lo.ptId = P.id Join doctors d on Lo.drId = d.id    where Lo.isDeleted = false and  radioFDId = "+ress.id,function(err,result){
            if(err){
                res.status(500).send(err.message)
            }else{
                res.send(result)
...
exports.findResultsByradioId = (req,res)=>{
    radioFD.findOne({
        ...
        db2.query("SELECT rs.name as radioName , d.firstName as OrderingDrFirstName , d.lastName as tOrderingDrLastName ,  Lo.id,Lo.result,Lo.comments,P.id as PtID,P.phone,P.firstname,P.secondName,P.lastname,P.address,P.phone from radioOrders Lo Join Patients P on Lo.ptId = P.id Join doctors d on Lo.drId = d.id  JOIN radios rs on Lo.radioId = rs.id  where Lo.result IS NOT NULL and Lo.isDeleted = false and  radioFDId = "+ress.id,function(err,result){
            if(err){
                res.status(500).send(err.message)
            }else{
                res.send(result)
